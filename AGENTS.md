...
- `third_party/bearssl/` — vendored BearSSL (MIT), cross-compiled into
  `libbearssl.a` for userland; x86 SIMD variants excluded (hosted-libc
  intrinsics headers), portable constant-time code used instead. `user/libc/`
  is the 5-function shim libc it links against. HTTPS lives entirely in
